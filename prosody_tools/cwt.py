...
from numpy import (asarray, arange, array, argsort, arctanh, ceil, concatenate, conjugate, cos, diff, exp, intersect1d, isnan, isreal, log, log2, mod, ones, pi, prod, real, round, sort, sqrt, unique, zeros, polyval, nan, ma, floor, interp, loadtxt, savetxt, angle, mean,hanning,convolve)
...
from scipy.signal import convolve2d, lfilter, medfilt
...
class DOG:
    ...
    def psi_ft(self, f):
        ...
        #self.m = 2
        #gam = 1.0j*f*(np.math.factorial(self.m-1))  / (1.0+(1j*(f-1)**(self.m)))
        k = 1.0 # normalization constant
        f0 =0.01  # center frequency (what units?)
        l = f0*3 #damping factor
        #gam = k*(np.math.factorial(self.m-1)) / (-f+(l - 1j*2*3.14156*f0))**self.m



        #k =  1 # normalization constant
        #l = 0.1  #damping factor
        #f0 = 0.2  # central freq
        # best so far gam = 1j*f*(np.math.factorial(self.m-1)) / (-f+(l - 1j*2*3.14156*f0))**self.m

        #gam = (1j*f*(np.math.factorial(self.m-1)) / (-f+(l - 1j*2*3.14156*f0))**self.m)
       # gam = (1j*f*(np.math.factorial(self.m-1)) / (-f+(l - 1j*2*3.14156*f0))**(3))
        #for x in range (0,10):
        #gam = 1j*f*(np.math.factorial(self.m-1)) / (f-(l-1j*2*3.14156*f0))**self.m
        #gam = 1j*f*(np.math.factorial(self.m-1)) / (-f+(l-1j*2*3.14156*f0))**self.m
        #gam = (1+(1j*(f-f0))/0.1)**31

        
        if self.m > 1:
            dog = (2*((-1j ** (self.m)/ sqrt(gamma(self.m+0.5)) * f ** (self.m) *
                        exp(- 0.5 * f ** 2)))-((-1j ** (self.m-1)/ sqrt(gamma(self.m-1+0.5)) * f ** (self.m-1) *
                        exp(- 0.5 * f ** 2))))/3.0
            gam = dog
              
        #pylab.plot(np.roll(gamma, 1000)[500:1500], 'red')
        #pylab.clf()
        #pylab.plot(np.roll(ifft(gam),1000))
        #raw_input()
    ...
    def psi(self, t):
        ...
        pylab.plot((-1) ** (self.m + 1) * polyval(p, t) * exp(-t ** 2 / 2) / sqrt(gamma(self.m + 0.5)))
        raw_input()
...
def cwt(signal, dt=1., dj=1./12, s0=-1, J=-1, wavelet=Morlet(), result=None, f0=None):
    ...
    W = zeros((len(sj), N), 'complex')
        #signal = f0.interpolate_zeros(signal,'linear', mean(signal))
    ...
    for n, s in enumerate(sj):
        ...
        sigmat.append(array(signal))
    
        signal_ft = fft(signal, N)
        #pylab.plot(cwt_utils.normalize(signal))
        #pylab.plot(cwt_utils.normalize(diff(signal, 1))+5)
        #pylab.plot((cwt_utils.normalize(diff(signal, 2))+5))
        #pylab.plot((cwt_utils.normalize(diff(signal, 3))+15))
        #pylab.draw()
