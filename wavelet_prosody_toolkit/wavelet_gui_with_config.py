...
class SigWindow(QtWidgets.QDialog):
    ...
    def setF0Limits(self):
        ...
        self.voicing.setSliderPosition(self.config["voicing_threshold"])
    ...
    def prosodicFeats(self):
        ...
        self.wF0 = QtWidgets.QLineEdit(str(self.config["weights"]["f0"]))
        self.wF0.setInputMask("0.0")
        self.wF0.setMaxLength(3)

        # Energy widgets
        l2 = QtWidgets.QLabel("Energy")
        self.wEnergy = QtWidgets.QLineEdit(str(self.config["weights"]["energy"]))
        self.wEnergy.setInputMask("0.0")
        self.wEnergy.setMaxLength(3)

        # Duration widgets
        l3 = QtWidgets.QLabel("Duration")
        self.wDuration = QtWidgets.QLineEdit(str(self.config["weights"]["duration"]))
    ...
    def populateTierList(self):
        ...
        if self.current_tier == "":
            self.current_tier = self.config["annotation_tier"]
        ...
        if len(self.current_dur_tiers)==0:
            self.current_dur_tiers = self.config["duration_tiers"]
        if len(self.current_dur_tiers) > 0:
    ...
    def analysis(self):
            ...
            if raw_pitch is None:
                ...
                if self.config["pitch_tracker"] == "REAPER":
                    try:
                        ...
                        self.logger.debug("REAPER not available, reverting to instantenous frequency pitch tracker.")
                        pass
                    
                if self.config["pitch_tracker"] == "inst_freq" or raw_pitch is None:
        ...
        if self.fUpdate['loma'] and labels:
            ...
            pos_loma_start_scale = unit_scale + int(self.config["prom_loma_start"]/scale_dist) # three octaves down from average unit length
            pos_loma_end_scale = unit_scale +  int(self.config["prom_loma_end"]/scale_dist)
            neg_loma_start_scale = unit_scale + int(self.config["boundary_loma_start"]/scale_dist)  # two octaves down
            neg_loma_end_scale = unit_scale + int(self.config["boundary_loma_end"]/scale_dist)  # one octave up
    ...
    def configuration(self):
        """ parse arguments, setup logging and read configuration file
        """
...
def main():
    ...
    try:
        """
        ### antti: moved the parsing inside the QT app, otherwise passing config to app is difficult
